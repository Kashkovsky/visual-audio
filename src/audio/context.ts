...
export namespace Sound {
	...
	const ctor = window.AudioContext || (window as any).webkitAudioContext as AudioContext || (window as any).mozAudioContext as AudioContext
	export const create: Lazy<Sound> = () => {
		const context = new ctor()
		const observableContext = context.state !== 'running' ? Rx.fromEvent(document, 'mousedown').pipe(
			Rx.switchMap(() => {
	...
	export namespace UserMedia {
		export const create: RTE.ReaderTaskEither<MediaStreamConstraints, Error, MediaStream> =
			constr => TE.tryCatch(() => navigator.mediaDevices.getUserMedia(constr), (e: Error) => new Error(e.message))
	}
	...
	export namespace AnalysedNode {
		...
		export const connectToSource = (analyser: AnalysedNode<AnalyserNode>) => (source: AudioNode): AnalysedNode => ({
			...analyser,
			node: source.connect(analyser.node)
		})

		export const connectToDestination = (dest: AudioNode) => (analyser: AnalysedNode<AnalyserNode>): AnalysedNode => ({
			...analyser,
			node: analyser.node.connect(dest)
		})

		export const create = ({ maxDecibels, minDecibels, smoothingTimeConstant = 0.85, fftSize = 512 }: AnalyserConfig) =>
		...
		export const fromUserMediaToOut = (config: AnalyserConfig) =>
			(ctx: AudioContext): Rx.Observable<Sound.AnalysedNode> =>
				pipe(
					...
					t => Rx.from(t()),
					Rx.switchMap(E.fold(err => {
						console.error(err)
						return Rx.EMPTY
					}, node => Rx.of(node)))
				)

		export const attachAnimation = (animation: R.Reader<AnalysedNode, Rx.Observable<AnalysisData>>) => (analyzedNode: Rx.Observable<AnalysedNode>) =>
			analyzedNode.pipe(
				Rx.switchMap(animation)
			)
	}
